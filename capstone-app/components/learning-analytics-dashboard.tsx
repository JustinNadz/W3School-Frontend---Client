...
export default function LearningAnalyticsDashboard() {
  ...
  const updateAnalytics = () => {
    ...
    weeks.forEach((week, index) => {
      weeklyProgress.push({
        week,
        html: Math.min(20 + index * 15 + Math.random() * 10, 100),
        css: Math.min(10 + index * 12 + Math.random() * 8, 100)
      })
    })
    ...
    localStorage.setItem('learningAnalytics', JSON.stringify(newAnalytics))
  }
